...
if mode == "World Builder":
    ...
    with tab3:
        ...
        if st.button("Create New Region from Journal"):
            ...
            all_npcs = [ch_data['npc'] for ch_data in st.session_state.characters]
            all_quests = [ch_data['quest'] for ch_data in st.session_state.characters]
            ...
            prompt = (
                ...
                f"Return a JSON object with the keys 'name' and 'description'. The 'description' should itself be a JSON object "
                f"with keys: 'terrain', 'climate', 'special_features', 'quests', 'npcs'. Be immersive and detailed. "
                f"ONLY RETURN A JSON OBJECT, DO NOT ADD ANY EXTRA TEXT.\n\n"
                f"World Journal:\n{journal_text}\n\n"
            ...
            try:
                ...
                st.error("AI did not return valid JSON. Cannot create region.")
                region_data = {"name": "Unknown Region", "description": {}}
    
            st.session_state.regions.append(region_data)
            st.success(f"Region '{region_data.get('name', 'Unknown')}' created!")
    
        # Display all regions
        for idx, region in enumerate(st.session_state.regions):
            exp = st.expander(f"{region.get('name', 'Unknown Region')}", expanded=False)
            with exp:
